(function() {
  ...
  this.Tbl = class Tbl {
    ...
    * walk_relation_lines(cfg) {
      var col_names, error, error_name, limit, name, order_by, qname_i, query, ref, ref1, row_count, sample_row, schema, schema_i, sql, type;
      ...
      ({type, sql} = this.dba.first_row(this.dba.query(SQL`select type, sql from ${schema_i}.sqlite_schema
where name = $name
limit 1;`, {name})));
      try {
        // #.......................................................................................................
        ...
        /* get column names */
        /* TAINT implement in `Dba` */
        sample_row = this.dba.first_row(this.dba.query(SQL`select * from ${qname_i} limit 1`));
        if (sample_row == null) {
        ...
        error = error1;
        error_name = (ref = (ref1 = error.name) != null ? ref1 : error.code) != null ? ref : 'error';
      ...
      yield "\n";
      if (row_count > limit) {
        yield (CND.green(CND.reverse('   '))) + CND.steel(` ${type} ${qname_i} (${row_count} rows; first ${limit} shown)`);
      } else {
        yield (CND.green(CND.reverse('   '))) + CND.steel(` ${type} ${qname_i} (all ${row_count} rows)`);
      }
