(function() {
  ...
  this.Tbl = class Tbl {
    ...
    * _walk_relation_lines(cfg) {
    ...
    //---------------------------------------------------------------------------------------------------------
    dump_db(cfg) {
      var limit, line, name, order_by, ref1, ref2, ref3, schema, schema_i, title, y;
      validate.dbatbl_dump_db_cfg(cfg = {...types.defaults.dbatbl_dump_db_cfg, ...cfg});
      ({schema, order_by, limit} = cfg);
      schema_i = this.dba.sql.I(schema);
      //.......................................................................................................
      title = SQL`dump of SQLite DB at ${this.dba._schemas[schema].path}`;
      echo();
      echo(CND.white(title));
      echo(CND.white('—'.repeat(width_of(title))));
      echo();
      ref1 = this._walk_relation_lines({
        name: 'sqlite_schema',
        limit: null
      });
      for (line of ref1) {
      ...
      ref2 = this.dba.query(SQL`select * from ${schema_i}.sqlite_schema
where type in ( 'table', 'view' )
order by type, name;`);
      for (y of ref2) {
        ({name} = y);
        ref3 = this._walk_relation_lines({schema, name, order_by, limit});
        for (line of ref3) {
          echo(line);
        }
      }
